...
An implementation of **`Tenzies Game`** in `ReactJS`. While creating this project I learned about `Event Listeners in React`, `React State`, `Conditional Rendering in React`, `React Hooks(useEffect)`, etc. A player could track the number of rolls, current time and best time it took to win the game. Have Fun :smile:. After creating the project, it was deployed to `GitHub Pages`.
...
-   [x] If tenzies is `true`, change the button text to "Nouvelle partie" and use the `react-confetti` [package](https://www.npmjs.com/package/react-confetti) to render the <Confetti /> component.
